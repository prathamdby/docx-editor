...
export default function DocumentEditor() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    practicals.forEach((practical, pIndex) => {
      ...
      formDataObj.append(`practical_${pIndex}_conclusion`, practical.conclusion)

      practical.questions.forEach((question, qIndex) => {
        formDataObj.append(`practical_${pIndex}_question_${qIndex}_number`, question.number)
        formDataObj.append(`practical_${pIndex}_question_${qIndex}_questionText`, question.questionText)
        formDataObj.append(`practical_${pIndex}_question_${qIndex}_code`, question.code)
      })
    ...
    setPracticals((prev) =>
      prev.map((p, i) => {
        if (i === practicalIndex) {
          return {
            ...p,
            questions: [...p.questions, { number: String(p.questions.length + 1), questionText: "", code: "" }],
          }
        }
        return p
      }),
  ...
  const handleFileChange = (practicalIndex: number, e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        <div className="py-20 text-center relative">
          ...
          >
            ...
            <p className="text-lg text-gray-400 max-w-2xl mx-auto">
              Generate beautifully formatted practical documents in seconds with our intelligent document generator.
            </p>
          </motion.div>
        </div>
                        ...
                        >
                          <Card className="p-4 space-y-4 bg-white/5 border-white/10">
                            <div className="flex justify-between items-center">
                              <h3 className="text-xl font-bold text-white">Practical {practical.practicalNo}</h3>
                              {practicals.length > 1 && (
                                <Button
                                  type="button"
                                  variant="ghost"
                            ...
                            <div>
                              <Label htmlFor={`practicalNo-${pIndex}`} className="text-white">
                                Practical Number
                              </Label>
                              <Input
                                id={`practicalNo-${pIndex}`}
                                value={practical.practicalNo}
                                onChange={(e) => handlePracticalChange(pIndex, "practicalNo", e.target.value)}
                                required
                                className="mt-1.5 bg-white/[0.07] border-white/10 text-white focus:bg-white/10"
                              />
                            </div>
                            <div>
                              <Label htmlFor={`aim-${pIndex}`} className="text-white">
                                Aim
                              </Label>
                              <Input
                                id={`aim-${pIndex}`}
                                value={practical.aim}
                                onChange={(e) => handlePracticalChange(pIndex, "aim", e.target.value)}
                                required
                                className="mt-1.5 bg-white/[0.07] border-white/10 text-white focus:bg-white/10"
                              />
                            </div>
                                    ...
                                    <Card className="p-3 bg-white/5 border-white/10">
                                      <div className="flex justify-between items-start mb-2">
                                        <div className="w-full mr-2">
                                          <Label htmlFor={`question-${pIndex}-${qIndex}`} className="text-white">
                                            Question Number
                                          </Label>
                                          <Input
                                            ...
                                            onChange={(e) =>
                                              handleQuestionChange(pIndex, qIndex, "number", e.target.value)
                                            }
                                            required
                                            className="mt-1.5 bg-white/[0.07] border-white/10 text-white focus:bg-white/10"
                                          />
                                      ...
                                      <div className="space-y-3">
                                        <div>
                                          <Label htmlFor={`questionText-${pIndex}-${qIndex}`} className="text-white">
                                            Question
                                          </Label>
                                          <Textarea
                                            ...
                                            onChange={(e) =>
                                              handleQuestionChange(pIndex, qIndex, "questionText", e.target.value)
                                            }
                                            className="mt-1.5 bg-white/[0.07] border-white/10 text-white focus:bg-white/10"
                                            required
                                        ...
                                        <div>
                                          <Label htmlFor={`code-${pIndex}-${qIndex}`} className="text-white">
                                            Code
                                          </Label>
                                          <Textarea
                                            ...
                                            value={question.code}
                                            onChange={(e) =>
                                              handleQuestionChange(pIndex, qIndex, "code", e.target.value)
                                            }
                            ...
                            <div>
                              <div className="flex justify-between items-center mb-2">
                                ...
                                <span className="text-sm text-gray-400">{practical.outputs.length}/3</span>
                              </div>
                                ...
                                {practical.outputs.length < 3 && (
                                  <div>
                                    ...
                                    <Button
                                      ...
                                      onClick={() => document.getElementById(`output-${pIndex}`)?.click()}
                                    >
                            ...
                            {/* Conclusion Section */}
                            <div>
                              <Label htmlFor={`conclusion-${pIndex}`} className="text-white">
                                Conclusion
                              </Label>
                              <Textarea
                                id={`conclusion-${pIndex}`}
                                value={practical.conclusion}
                                onChange={(e) => handlePracticalChange(pIndex, "conclusion", e.target.value)}
                      ...
                      {practicals.map((practical, pIndex) => (
                        <div key={pIndex} className="space-y-6 mt-8">
                          <div className="text-center">
                            <h1 className="text-xl font-bold underline">PRACTICAL No. {practical.practicalNo}</h1>
                          </div>

                          <div>
                            <p>
                              <span className="font-bold underline">AIM:</span> {practical.aim}
                            </p>
                          </div>
                          ...
                          {practical.questions.map((question, qIndex) => (
                            <div key={qIndex} className="space-y-4">
                              <div>
                                <p className="font-bold underline">Question {question.number}:</p>
